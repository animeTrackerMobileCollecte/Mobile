...
export default function WishlistScreen() {
  const { animeData, removeFromWishlist, startWatching, toggleFavorite } =
    useAnime();

  const { isDarkMode } = useTheme();
  const theme = isDarkMode ? COLORS.dark : COLORS.light;

  const wishlistAnimes = animeData.filter(
    (anime) => anime.status === "WISHLIST" || anime.isInWishlist === true
  );
